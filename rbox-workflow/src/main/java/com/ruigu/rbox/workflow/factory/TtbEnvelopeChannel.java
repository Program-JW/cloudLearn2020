...
@Slf4j
@Component
public class TtbEnvelopeChannel extends BaseEnvelopeChannel {
    ...
    {
        try {
            Class[] productClasses = TtbEnvelopeChannel.class.getDeclaredClasses();
            // 逐个创建并放入list
    ...
    public Boolean match(EnvelopeChannelEnum channel) {
        return EnvelopeChannelEnum.WORKFLOW.equals(channel);
    }
    ...
    class WeixinTextEnvelop implements EnvelopeProduct {
        ...
        public EnvelopeReq product() {
            ...
            // 设置特殊参数
            envelopeReq.setSource(hedwigConfigProperties.getTtbChannel().getSource());
            envelopeReq.setAccount(hedwigConfigProperties.getTtbChannel().getAccount());
            envelopeReq.setChannel(Collections.singletonList(hedwigConfigProperties.getTtbChannel().getWxChannel()));
            return envelopeReq;
        }
    }
